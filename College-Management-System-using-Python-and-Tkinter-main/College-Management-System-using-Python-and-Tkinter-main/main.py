...
def draw_login_page():
    ...
    #heading label
    college=Label(header, text="SMIU College",font=("Helvetica",16,"bold"), bg = "#e23946",fg="#eae2b7")
    college.place(x=0, y=10, width=400)
...
def draw_execute_dbms():
    os.system('python edit_courses.py')
...
def draw_visitor():
    ...
    image1 = Image.open("media/image.png")
    resized_image = image1.resize((1080, 400), Image.Resampling.LANCZOS)  # Use LANCZOS for high-quality resizing
    test = ImageTk.PhotoImage(resized_image)
    
    label1 = Label(dashboard, image=test)
    label1.photo = test  # Keep a reference to avoid garbage collection
    label1.place(x=0, y=0, height=400, width=1080)  # Adjust placement dimensions
    ...
    option.place(x=730, y=270, width=200, height=50)
...
def draw_admin():
    ...
    # Open and resize the image
    image1 = Image.open("media/image.png")
    resized_image = image1.resize((1080, 400), Image.Resampling.LANCZOS)  # Resize with high-quality resampling
    test = ImageTk.PhotoImage(resized_image)
    ...
    option.place(x=420, y=170, width=250, height=50)
    option = Button(dashboard, text="View/Edit Courses", command=draw_execute_dbms, bd=0, font=("Helvetica", 16), bg="#118ab2", fg="#eae2b7")
    option.place(x=740, y=275, width=250, height=50)
...
#heading label
college=Label(header, text="SMIU College",font=("Helvetica",36,"bold"), bg = "brown",fg="#eae2b7")
college.place(x=105, y=20, width=950)
